...
xml_tree_t *xml_creat(const char *fname, xml_option_t *opt)
{
    ...
    if (NULL == buff)
    {
        sys_error("Load xml file into memory failed![%s]", fname);
        return NULL;
    }
...
xml_tree_t *xml_screat(const char *str, xml_option_t *opt)
{
    ...
    do
    {
        ...
        {
            sys_error("Init xml stack failed!");
            break;
        }
        ...
        if (NULL == xml)
        {   
            sys_error("Init xml tree failed!");
            break;
        }
...
int xml_fprint(xml_tree_t *xml, FILE *fp)
{
    ...
    if (NULL == child) 
    {
        sys_error("The tree is empty!");
        return XML_ERR_EMPTY_TREE;
    }
    
    if (stack_init(&stack, XML_MAX_DEPTH))
    {
        sys_error("Stack init failed!");
        return XML_ERR_STACK;
    }
...
int xml_fwrite(xml_tree_t *xml, const char *fname)
{
    ...
    if (NULL == fp)
    {
        sys_error("Call fopen() failed![%s]", fname);
        ...
        const char *name, const char *value)
{
    ...
    if (NULL == parent)
    {
        sys_error("Please create root node at first!");
        return NULL;
    }

    if (xml_is_attr(node))
    {
        sys_error("Can't add attr for attribute node!");
        return NULL;
    }
    ...
    sys_error("Add attr node failed!");
    return NULL;
}
...
xml_node_t *xml_add_child(xml_tree_t *xml, xml_node_t *node, const char *name, const char *value)
{
    ...
    if (xml_is_attr(node))
    {
        sys_error("Can't add child for attribute node![%s]", node->name);
        return NULL;
    }
#if defined(__XML_EITHER_CHILD_OR_VALUE__)
    else if (xml_has_value(node))
    {
        sys_error("Can't add child for the node which has value![%s]", node->name);
        return NULL;
    }
    ...
    if (NULL == child)
    {
        sys_error("Create node failed![%s]", name);
        return NULL;
    }
...
int xml_node_length(xml_tree_t *xml, xml_node_t *node)
{
    ...
    if (length < 0)
    {
        sys_error("Get the length of node failed!");
        stack_destroy(&stack);
        return -1;
    }
...
int _xml_pack_length(xml_tree_t *xml, xml_node_t *node)
{
    ...
    switch(node->type)
    {
        case XML_NODE_CHILD: /* 处理孩子节点 */
        {
            ...
            if (length < 0)
            {
                sys_error("Get length of the node failed!");
                stack_destroy(&stack);
                return -1;
            }
            break;
