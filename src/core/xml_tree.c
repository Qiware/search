...
xml_tree_t *xml_creat_empty(void)
{
    ...
    /* 2. Add root node */
    xml->root = xml_node_creat(XML_NODE_ROOT);
    if (NULL == xml->root)
    {
        free(xml), xml = NULL;
        LogError("Calloc failed!");
...
xml_tree_t *xml_creat(const char *fname)
{
    ...
    if (NULL == buff)
    {
        LogError("Load xml file into memory failed![%s]", fname);
        return NULL;
    }
 ...
 xml_tree_t *xml_screat(const char *str)
{
    ...
    do
    {
        ...
        if (XML_SUCCESS != ret)
        {
            LogError("Init xml stack failed!");
            break;
        }
        ...
        if (XML_SUCCESS != ret)
        {
            LogError("Parse xml file cache failed!");
            xml_destroy(xml);
            break;
        }
    }while (0);
    ...
    {   \
        ...
        {   \
            ...
            {   \
                ...
                xml_node_sfree(child), child = NULL;    \
                continue;   \
            }   \
...
int xml_node_free(xml_node_t *node)
{
    ...
    {
        ret = xml_delete_child(parent, node);
        if (XML_SUCCESS != ret)
        {
            return ret;
        }
    ...
    do
    {
        ...
        current = xml_free_next(stack, current); 
...
int xml_fwrite(xml_tree_t *xml, const char *fname)
{
    ...
    if (NULL == fp)
    {
        LogError("Call fopen() failed![%s]", fname);
...
xml_node_t *xml_rsearch(xml_node_t *curr, const char *path)
{
    ...
    if (NULL == node)
    {
        LogError("Path is not exist! path:[%s]", path);
        return NULL;
    }
...
xml_node_t *xml_add_attr(xml_node_t *node, const char *name, const char *value)
{
    ...
    if (NULL == parent)
    {
        LogError("Please create root node at first!");
        return NULL;
    }

    if (xml_is_attr(node))
    {
        LogError("Can't add attr for attribute node!");
        return NULL;
    }

    /* 1. 创建节点 */
    attr = xml_node_creat_ext(XML_NODE_ATTR, name, value);
    if (NULL == attr)
    {
        LogError("Create node failed!");
        return NULL;
    }
...
xml_node_t *xml_add_child(xml_node_t *node, const char *name, const char *value)
{
    ...
    if (xml_is_attr(node))
    {
        LogError("Can't add child for attribute node![%s]", node->name);
        return NULL;
    }
#if defined(__XML_OCOV__)
    else if (xml_has_value(node))
    {
        LogError("Can't add child for the node which has value![%s]", node->name);
        return NULL;
    }
#endif /*__XML_OCOV__*/

    /* 1. 新建孩子节点 */
    child = xml_node_creat_ext(XML_NODE_CHILD, name, value);
    if (NULL == child)
    {
        LogError("Create node failed![%s]", name);
        return NULL;
    }
...
xml_node_t *xml_add_node(xml_node_t *node, const char *name, const char *value, int type)
{
    switch(type)
    {
        case XML_NODE_ATTR:
        {
            return xml_add_attr(node, name, value);
        }
        case XML_NODE_ROOT:
        case XML_NODE_CHILD:
        {
            return xml_add_child(node, name, value);
        }
    }
...
int xml_node_length(xml_node_t *node)
{
    ...
    length = _xml_node_length(node, &stack);
    if (length < 0)
    {
        LogError("Get the length of node failed!");
        stack_destroy(&stack);
        return -1;
    }
...
int _xml_pack_length(xml_node_t *node)
{
    ...
    switch(node->type)
    {
        ...
        {
            length = xml_pack_node_length(node, &stack);
            if (length < 0)
            {
                LogError("Get length of the node failed!");
                stack_destroy(&stack);
                return -1;
            }
            break;
        ...
        case XML_NODE_ROOT:  /* 处理父亲节点 */
        {
            ...
            while (NULL != child)
            {
                length2 = xml_pack_node_length(child, &stack);
                if (length2 < 0)
                {
                    LogError("Get length of the node failed!");
                    stack_destroy(&stack);
