...
static int rtrd_lsn_cmd_query_conf_hdl(rtrd_cntx_t *ctx, rtrd_lsn_t *lsn, rtdt_cmd_t *cmd);
static int rtrd_lsn_cmd_query_recv_stat_hdl(rtrd_cntx_t *ctx, rtrd_lsn_t *lsn, rtdt_cmd_t *cmd);
static int rtrd_lsn_cmd_query_proc_stat_hdl(rtrd_cntx_t *ctx, rtrd_lsn_t *lsn, rtdt_cmd_t *cmd);
...
{
#define RTDT_LSN_TMOUT_SEC 30
#define RTDT_LSN_TMOUT_USEC 0
    fd_set rdset;
    int ret, max;
    rtrd_lsn_t *lsn;
    ...
    for (;;)
    {
        ...
        timeout.tv_sec = RTDT_LSN_TMOUT_SEC;
        timeout.tv_usec = RTDT_LSN_TMOUT_USEC;
...
static int rtrd_lsn_accept(rtrd_cntx_t *ctx, rtrd_lsn_t *lsn)
{
    ...
    rtdt_cmd_add_sck_t *args = (rtdt_cmd_add_sck_t *)&cmd.args;
...
static int rtrd_lsn_cmd_query_conf_hdl(rtrd_cntx_t *ctx, rtrd_lsn_t *lsn, rtdt_cmd_t *cmd)
{
    ...
    rtdt_cmd_conf_t *args = (rtdt_cmd_conf_t *)&rep.args;

    memset(&rep, 0, sizeof(rep));

    /* 1. 设置应答信息 */
    rep.type = RTDT_CMD_QUERY_CONF_REP;
...
static int rtrd_lsn_cmd_query_proc_stat_hdl(rtrd_cntx_t *ctx, rtrd_lsn_t *lsn, rtdt_cmd_t *cmd)
{
    int idx;
    rtdt_cmd_t rep;
    const rtdt_worker_t *wrk = (rtdt_worker_t *)ctx->worktp->data;
    rtdt_cmd_proc_stat_t *stat = (rtdt_cmd_proc_stat_t *)&rep.args;

    for (idx=0; idx<ctx->conf.work_thd_num; ++idx, ++wrk)
    {
        /* 1. 设置应答信息 */
        rep.type = RTDT_CMD_QUERY_PROC_STAT_REP;
