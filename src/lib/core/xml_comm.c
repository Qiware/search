...
int xml_parse(xml_tree_t *xml, Stack_t *stack, const char *str)
{
    ...
    while (!XmlIsStrEndChar(*(parse.ptr)))
    {
        ...
        switch(*(parse.ptr))
        {
            case XML_BEGIN_FLAG:
            {
                switch(*(parse.ptr+1))
                {
                    case XML_VERS_FLAG:  /* "<?" 版本开始 */ 
                    {
                        ...
                        {
                            plog_error("XML format is wrong![%-.32s] [%ld]",
                    ...
                    case XML_NOTE_FLAG:   /* "<!--" 注释信息 */
                    {
                        ...
                        {
                            plog_error("XML format is wrong![%-.32s]", parse.ptr);
                            return XML_ERR_FORMAT;
                        }
                        break;
                    }
                    ...
                    default:    /* "<XYZ" 节点开始 */
                    {
                        ...
                        {
                            plog_error("Parse XML failed! [%-.32s] [%ld]",
                                parse.ptr, parse.ptr-parse.str);
                            return XML_ERR_FORMAT;
                        }
                        break;
            ...
            case STR_END_FLAG:  /* 字串结束'\0' */
            {
                ...
                {
                    plog_debug("Parse xml success!");
                    return XML_OK;
                }
                plog_error("Invalid format! [%-.32s] [%ld]", parse.ptr, parse.ptr-parse.str);
                return XML_ERR_FORMAT;
            }
            default:            /* 非法字符 */
            {
    ...
    if (!stack_isempty(stack))
    {
        plog_error("Invalid format! [%-.32s]", parse.ptr);
        return XML_ERR_FORMAT;
    }
...
static int xml_parse_mark(xml_tree_t *xml, Stack_t *stack, xml_parse_t *parse)
{
    ...
    {
        plog_error("Get mark name failed!");
        return XML_ERR;
    }
    ...
    if (xml_mark_has_attr(parse))
    {
        ...
        {
            plog_error("Get mark attr failed!");
            return XML_ERR;
        }
    }
...
static int xml_mark_get_name(xml_tree_t *xml, Stack_t *stack, xml_parse_t *parse)
{
    ...
    if (NULL == node)
    {
        plog_error("Create xml node failed!");
    ...
    {
        plog_error("XML format is wrong!\n[%-32.32s]", parse->ptr);
        return XML_ERR_FORMAT;
    }
...
static int xml_mark_get_attr(xml_tree_t *xml, Stack_t *stack, xml_parse_t *parse)
{
    ...
    if (NULL == top)
    {
        plog_error("Get stack top failed!");
        return XML_ERR_STACK;
    }
    ...
    do
    {
        ...
        {
            errflg = 1;
            plog_error("Attribute format is incorrect![%-.32s]", parse->ptr);
            break;
        }
        ptr++;                                  /* 跳过"=" */
        while (XmlIsIgnoreChar(*ptr)) { ++ptr; }/* 跳过=之后的无意义字符 */
        ...
        while ((*ptr != border) && !XmlIsStrEndChar(*ptr))/* 计算 双/单 引号之间的数据长度 */
        {
            ...
            if (XmlIsAndChar(*ptr))
            {
                ...
                {
                    errflg = 1;
                    plog_error("Parse forwad string failed!");
                    break;
                }
        ...
        {
            errflg = 1;
            plog_error("Mismatch border [%c]![%-.32s]", border, parse->ptr);
            break;
        }
        ...
        if (NULL != split.head)
        {
            ...
            {
                errflg = 1;
                plog_error("Alloc memory failed!");
                break;
            }
...
static int xml_mark_get_value(xml_tree_t *xml, Stack_t *stack, xml_parse_t *parse)
{
    ...
    else if (XmlIsStrEndChar(*p1))  /* 为结束符时 */
    {
        ...
        xml_esc_free(&split);
    #endif /*__XML_ESC_PARSE__*/
        plog_error("XML format is wrong! MarkName:[%s]", curr->name);
        return XML_ERR_FORMAT;
    }
    else                            /* 为单引号或双引号时 */
    {
        ...
        {
            plog_error("XML format is wrong! [%s]", p2);
            return XML_ERR_FORMAT;
        }
    }
    ...
    plog_error("XML format is wrong: Node have child and value at same time!");
    return XML_ERR_FORMAT;
...
int xml_delete_child(xml_tree_t *xml, xml_node_t *node, xml_node_t *child)
{
    ...
    {
        plog_error("Parent node is not right!");
        return XML_ERR_PTR;
    }
...
int _xml_node_length(xml_tree_t *xml, xml_node_t *root, Stack_t *stack)
{
    ...
    if (0 != depth)
    {
        plog_error("Stack depth must empty. depth:[%d]", depth);
        return XML_ERR_STACK;
    }
    ...
    const char *str, int len, xml_esc_split_t *split)
{
    ...
    if (NULL == node)
    {
        plog_error("Calloc memory failed!");
        return XML_ERR_CALLOC;
    }
