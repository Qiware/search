...
int shm_slab_init(shm_slab_pool_t *pool)
{
    ...
    plog_info("min_size:%d min_shift:%d slot_off:%d page_off:%d page_num:%d ",
...
void shm_slab_dealloc(shm_slab_pool_t *pool, void *p)
{
    ...
    {
        plog_error("Pointer address is incorrect!");
        return;
    }
    ...
    switch(page->type)
    {
        ...
        default:
        {
            plog_error("Type is unknown! [%d]", page->type);
            break;
        }
    }
