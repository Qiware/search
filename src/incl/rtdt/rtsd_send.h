...
{
    int devid;                          /* 设备ID: 唯一值 */
    char name[RTDT_NAME_MAX_LEN];       /* 发送端名称 */

    rtdt_auth_conf_t auth;              /* 鉴权信息 */
    ...
    rtdt_cpu_conf_t cpu;                /* CPU亲和性配置 */

    rtdt_queue_conf_t sendq;            /* 发送队列配置 */
...
typedef struct
{
    ...
    rtdt_reg_t reg[RTDT_TYPE_MAX];      /* 回调注册对象(TODO: 如果类型过多，可构造平衡二叉树) */
    queue_t **recvq;                    /* 接收队列 */
} rtsd_cntx_t;
